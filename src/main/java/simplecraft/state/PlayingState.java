 ...
 * Uses the active world from {@code SimpleCraft.getActiveWorld()} for world name and seed.<br>
 * Creates a dynamically loaded world via {@code World.update(Vector3f, int)}.<br>
 ...
 * The {@link simplecraft.world.DayNightCycle} modulates sky brightness and tint, which are applied to<br>
...
public class PlayingState extends FadeableAppState
{
	...
	/** Y coordinate to scan downward from when looking for terrain (world ceiling for surface, lower for campfire). */
	...
	protected void initialize(Application app)
	{
		...
		final GameStateManager gameStateManager = simpleCraft.getGameStateManager();
		final GameInputManager gameInputManager = simpleCraft.getGameInputManager();
		...
		_pauseListener = (String name, boolean isPressed, float tpf) ->
		{
			...
			final GameState currentState = gameStateManager.getCurrentState();
			...
			if (currentState == GameState.PLAYING && !_pendingSpawn && !_waitingForGround && !_playerDead && !QuestionManager.isActive())
			{
				...
				_paused = true;
				gameStateManager.switchTo(GameState.PAUSED);
			}
		};
		
		gameInputManager.addTrackedListener(_pauseListener, GameInputManager.PAUSE);
		...
		_inventoryListener = (String name, boolean isPressed, float tpf) ->
		{
			...
			if (gameStateManager.getCurrentState() != GameState.PLAYING || _pendingSpawn || _waitingForGround || _playerDead || QuestionManager.isActive())
			{
				return;
			}
		...
		gameInputManager.addTrackedListener(_inventoryListener, GameInputManager.INVENTORY);
		...
		gameInputManager.addTrackedListener(_hideHudListener, GameInputManager.HIDE_HUD);
		...
		gameInputManager.addTrackedListener(_helpListener, GameInputManager.HELP_SCREEN);
	}
	...
	protected void onEnterState()
	{
		...
		final int renderDistance = settingsManager.getRenderDistance();
		...
		_dayNightCycle = new DayNightCycle((_playerSaveData != null ? _playerSaveData.getTimeOfDay() : STARTING_TIME_OF_DAY));
		...
		_textureAtlas.buildAtlas(assetManager);
		// _textureAtlas.saveDebugAtlas("debug_atlas.png"); // Save debug image when needed.
		final Material atlasMaterial = _textureAtlas.createMaterial(assetManager);
		...
		_world = new World((_activeWorld != null ? _activeWorld.getSeedValue() : 0), atlasMaterial);
		...
		final FlyByCamera flyByCamera = app.getFlyByCamera();
		...
		_spawnScanFromY = Region.SIZE_Y - 1;
	...
	public void update(float tpf)
	{
		...
		if ((_pendingDragonArenaEntry || _pendingShadowArenaEntry) && _world != null && _playerController != null)
		{
			...
			// Wait at least 2 frames so the loading screen is guaranteed to render before the heavy arena generation blocks the main thread.
			...
			// Force one PlayerController update so the camera snaps to the spawn position BEFORE the loading screen is removed. Without this, the camera is still at the old main-world coordinates for one frame, briefly showing a tower interior.
		...
		if (_pendingSpawn && _world != null)
		{
			...
			// The head clearance check pushes spawnY above the water surface, but gravity pulls the player right back into the water on the first frame.
			if (spawnY >= 0 && spawnY <= TerrainGenerator.WATER_LEVEL + 1)
			{
				if (_world.getBlock(_spawnTargetX, spawnY - 1, _spawnTargetZ).isLiquid())
				{
					spawnY = -1;
				}
			}
			...
			// Gate on center region loaded rather than spawnY - the spawn column may be underwater (spawnY rejected above) but loaded regions still exist to search.
			if (_newWorldSpawn && (_world.getRegion(Math.floorDiv(SPAWN_X, Region.SIZE_XZ), Math.floorDiv(SPAWN_Z, Region.SIZE_XZ)) != null))
			{
				...
				for (int step = 0; step < STEPS_PER_FRAME && !foundFlatGrassland; step++)
				{
					...
					// Don't advance _spawnSearchStep - retry this position next frame when the region is likely loaded.
			...
			// Without this, the player can be placed before terrain data exists, causing them to spawn inside the ground when blocks load around them.
			if (spawnY >= 0)
			{
				...
				if (_world.getBlock(_spawnTargetX, spawnY - 1, _spawnTargetZ).isLiquid() || spawnY <= TerrainGenerator.WATER_LEVEL)
				{
		...
		// The loading screen stays visible while the player falls from spawn height to the surface. World and player physics keep ticking behind the screen so terrain continues loading and gravity settles the player.
		if (_waitingForGround && _world != null && _playerController != null)
		{
			...
			_world.update(_playerController.getPosition(), groundApp.getSettingsManager().getRenderDistance());
		...
		if (_world != null && _playerController != null)
		{
			...
			if (_dayNightCycle != null)
			{
				...
				// These are discrete day/night values (not continuous) so all regions rebuild to the same brightness, preventing visual seams.
			...
			if (_musicManager != null && !isInBossArena())
			{
				...
				_musicManager.update(tpf, _playerController.isInWater(), _world.isUnderground((int) pos.x, (int) pos.y, (int) pos.z));
			}
			...
			else
			{
				...
				// Player update handles camera - when a screen is open, input is unregistered so no movement occurs, but we still need to call update for camera positioning.
				_playerController.update(tpf);
			...
			// Check enemies FIRST on left-click. If the crosshair is on an enemy, suppress block interaction's attack so we don't mine blocks behind enemies.
			// Skip combat when a screen is open or in the boss arena.
			final boolean attackHeld = (_blockInteraction != null) && _blockInteraction.isAttackHeld();
			boolean suppressBlockAttack = false;
			if (_combatSystem != null && _spawnSystem != null && !_playerDead && !screenOpen && !isInBossArena())
			{
				if (attackHeld)
				{
					suppressBlockAttack = _combatSystem.tryPlayerAttack(app.getCamera(), _spawnSystem.getEnemies(), _world, _playerController);
				}
			}
			...
			if (_viewmodelRenderer != null && attackHeld && !_playerDead && !screenOpen)
			{
			...
			if (_blockInteraction != null && !_playerDead && !screenOpen)
			{
				...
				_blockInteraction.setShowHighlight(settingsManager.isShowHighlight());
			...
			if (isInBossArena() && _bossArenaManager != null)
			{
				...
				if (boss != null && boss.isDying() && !_bossDeathProcessed)
				{
					...
					final boolean dragon = boss.getType() == EnemyType.DRAGON;
					MessageManager.show(LanguageManager.get("msg.boss_slain").replace("{0}", LanguageManager.get(dragon ? "hud.dragon" : "hud.shadow")), 5.0f);
					...
					System.out.println("PlayingState: " + (dragon ? "Dragon" : "Shadow") + " killed! Recall Orb spawned.");
				}
			...
			if (_playerController.isDead() && !_playerDead)
			{
				...
				if (_dropManager != null)
				{
					...
					for (int i = 0; i < Inventory.TOTAL_SLOTS; i++)
					{
						...
						if (item != null)
						{
							...
							_dropManager.spawnDrop(new Vector3f(deathPos.x + ((float) (Math.cos(angle) * radius)), deathPos.y, deathPos.z + ((float) (Math.sin(angle) * radius))), item.copy());
					...
					for (ArmorSlot slot : ArmorSlot.values())
					{
						...
						if (armor != null)
						{
							...
							final float radius = 0.25f + dropIndex * 0.07f;
							_dropManager.spawnDrop(new Vector3f(deathPos.x + ((float) (Math.cos(angle) * radius)), deathPos.y, deathPos.z + ((float) (Math.sin(angle) * radius))), armor.copy());
				...
				final InputManager deathInputManager = app.getInputManager();
				final String deathCause = _playerController.getDeathCause();
				if (_playerHUD != null)
				{
					_playerHUD.showDeathScreen(deathCause);
				}
				
				// Show cursor and register respawn click listener.
				deathInputManager.setCursorVisible(true);
				...
				if (!deathInputManager.hasMapping(ACTION_RESPAWN))
				{
					deathInputManager.addMapping(ACTION_RESPAWN, new MouseButtonTrigger(MouseInput.BUTTON_LEFT));
				}
				
				deathInputManager.addListener(_respawnListener, ACTION_RESPAWN);
				
				System.out.println("Player died! Cause: " + deathCause);
			}
			...
			{ // @formatter:off
				_playerHUD.update(
					...
					settingsManager.isShowCrosshair() && !screenOpen
				);
			} // @formatter:on
		}
	}
	...
	private void updateHeldTorchLight(float tpf)
	{
		...
		final ItemTemplate heldTemplate = (held != null) ? held.getTemplate() : null;
		final boolean holdingTorch;
		if (heldTemplate != null && heldTemplate.getType() == ItemType.BLOCK)
		{
			holdingTorch = (heldTemplate.getPlacesBlock() == Block.TORCH);
		}
		else
		{
			holdingTorch = false;
	...
	private void removeHeldTorchLight()
	{
		...
		{
			// Skip removal if a placed light-emitting block (torch, campfire) occupies this position - the held torch's light was absorbed by it and removing block light here would incorrectly erase the placed block's own light.
			if (_world.getBlock(_heldTorchLightPos[0], _heldTorchLightPos[1], _heldTorchLightPos[2]).getLightLevel() <= 0)
			{
	 ...
	 * The player controller already exists, so {@code finalizeSpawn(int)} detects<br>
	...
	private void performRespawn()
	{
		...
		// This happens here (on respawn click) instead of on death detection, so the death screen shows the arena behind it rather than the normal world.
		if (_bossArenaManager != null && _bossArenaManager.isInArena())
		{
			...
			{
				respawnAudio.fadeInMusic((_dayNightCycle.isNight() ? MusicManager.NIGHT_MUSIC_PATH : MusicManager.DAY_MUSIC_PATH), 2.0f);
			}
		}
		...
		System.out.println("Respawn target: [" + _spawnTargetX + ", " + respawnPoint.y + ", " + _spawnTargetZ + "] (campfire=" + hasCampfire + ")");
		
		// Pre-position the player at the respawn target immediately.
		// This prevents the player controller from having stale arena coordinates while the main world regions are loading during the pending spawn loop.
		...
		_spawnScanFromY = hasCampfire ? (int) respawnPoint.y : (Region.SIZE_Y - 1);
		...
		// Kick off region loading around the respawn position immediately, so background threads start generating terrain this frame rather than waiting until the first iteration of the pending spawn loop.
		_world.update(new Vector3f(_spawnTargetX, _spawnFallbackY, _spawnTargetZ), app.getSettingsManager().getRenderDistance());
	...
	protected void onExitState()
	{
		...
		if (_bossArenaManager != null && _bossArenaManager.isInArena())
		{
			...
			// Without this, the arena coordinates (200, 2, 145) would be saved as the main world position, causing the player to spawn underground on next load.
			if (_playerController != null)
			{
				final Vector3f respawnPoint = _playerController.getActiveRespawnPoint();
	...
	private void showLoadingScreen()
	{
		...
		final Geometry bgGeom = new Geometry("LoadingBg", new Quad(screenWidth, screenHeight));
		final AssetManager assetManager = app.getAssetManager();
		final Material bgMat = new Material(assetManager, "Common/MatDefs/Misc/Unshaded.j3md");
		...
		_loadingText = new BitmapText(FontManager.getFont(assetManager, FontManager.getTitlePath(), Font.PLAIN, fontSize));
		_loadingText.setText("Loading...");
		_loadingText.setSize(fontSize);
		_loadingText.setColor(new ColorRGBA(0.85f, 0.85f, 0.85f, 1.0f));
		
		_loadingText.setLocalTranslation((screenWidth - _loadingText.getLineWidth()) / 2f, (screenHeight + _loadingText.getLineHeight()) / 2f, 1);
	...
	private void updateLoadingScreen(float tpf)
	{
		...
		_loadingText.setLocalTranslation((camera.getWidth() - _loadingText.getLineWidth()) / 2f, (camera.getHeight() + _loadingText.getLineHeight()) / 2f, 1);
	}
	...
	{
		// Search up to 100,000 spiral steps - covers a radius of ~158 blocks, well beyond the loaded region boundary for typical render distances.
	...
	{
		final SimpleCraft app = SimpleCraft.getInstance();
		final InputManager spawnInputManager = app.getInputManager();
		...
		if (_playerController != null)
		{
			...
			// Ensure cursor is hidden for first-person control.
			spawnInputManager.setCursorVisible(false);
		...
		final AssetManager spawnAssetManager = app.getAssetManager();
		final AudioManager spawnAudioManager = app.getAudioManager();
		final Camera spawnCamera = app.getCamera();
		final Node rootNode = app.getRootNode();
		final SettingsManager spawnSettings = app.getSettingsManager();
		...
		_playerController = new PlayerController(spawnCamera, spawnInputManager, _world, spawnAudioManager);
		_playerController.setMouseSensitivity(spawnSettings.getMouseSensitivity());
		_playerController.setFov(spawnSettings.getFov());
		...
		if (_playerSaveData != null)
		{
			...
			final float savedX = _playerSaveData.getPosX();
			final float savedY = _playerSaveData.getPosY();
			final float savedZ = _playerSaveData.getPosZ();
			final float savedHealth = _playerSaveData.getHealth();
			_playerController.setPosition(savedX, savedY, savedZ);
			_playerController.setHealth(savedHealth);
			...
			System.out.println("Restored player from save: pos=[" + savedX + ", " + savedY + ", " + savedZ + "], health=" + savedHealth);
		}
		else
		{
		...
		// Must happen after player controller creation (which populates defaults) but before any gameplay systems that read inventory state.
		if (_inventorySaveData != null)
		{
			final simplecraft.item.Inventory inventory = _playerController.getInventory();
			inventory.deserialize(_inventorySaveData);
			
			// Strip any Recall Orb that may have been saved while in the arena (e.g. game crashed or force-quit after killing the boss but before using the orb).
			// The Recall Orb must never persist into the main world.
			for (int i = 0; i < 36; i++)
			{
				final simplecraft.item.ItemInstance item = inventory.getSlot(i);
		...
		_blockInteraction = new BlockInteraction(spawnCamera, spawnInputManager, _world, _playerController, spawnAssetManager, spawnAudioManager);
		_blockInteraction.registerInput();
		rootNode.attachChild(_blockInteraction.getOverlayNode());
		rootNode.attachChild(_blockInteraction.getDestructionEffectsNode());
		
		// Create the viewmodel renderer (first-person held item sprite on GUI).
		_viewmodelRenderer = new ViewmodelRenderer(spawnAssetManager, app.getRenderManager(), spawnCamera);
		...
		// Restore saved tile entities BEFORE attaching tile entity visual node, so particles and other visuals are created during deserialization.
		final TileEntityManager tileEntityManager = _world.getTileEntityManager();
		if (tileEntityManager != null)
		{
			...
			rootNode.attachChild(tileEntityManager.getNode());
		}
		...
		// FLAT_PANEL blocks (doors, windows) require TileEntityManager data for correct geometry (facing direction, open/closed state), which was unavailable during Initial background mesh building when the TileEntityManager was still empty.
		...
		_spawnSystem = new SpawnSystem(enemyNode, spawnAssetManager, _world.getSeed());
		_spawnSystem.setPlayerSpawnZone(_spawnTargetX, _spawnTargetZ);
		_spawnSystem.setDayNightCycle(_dayNightCycle);
		_spawnSystem.setAudioManager(spawnAudioManager);
		
		// Initialize the combat system (screen flashes, enemy -> player damage, player -> enemy attacks).
		_combatSystem = new CombatSystem(spawnAudioManager);
		
		// Initialize the drop manager (enemy death drops, pickup, despawn).
		_dropManager = new DropManager(spawnAssetManager, spawnAudioManager, _atlasMaterial);
		rootNode.attachChild(_dropManager.getNode());
		...
		_particleManager = new ParticleManager(spawnAssetManager);
		rootNode.attachChild(_particleManager.getNode());
		...
		{
			audioManager.fadeInMusic((_dayNightCycle.isNight() ? MusicManager.NIGHT_MUSIC_PATH : MusicManager.DAY_MUSIC_PATH), 3.0f);
	...
	private void createHUD()
	{
		...
		_craftingScreen = new CraftingScreen(_playerController, _blockInteraction, SimpleCraft.getInstance().getAudioManager());
		if (_blockInteraction != null)
		{
	...
	public void destroyWorld()
	{
		...
		// Safety net: if still in the boss arena when the world is being destroyed (e.g. quit-to-menu from pause while in arena), exit the arena first, restore the player to their respawn point and save.
		if (_bossArenaManager != null && _bossArenaManager.isInArena())
		{
			_bossArenaManager.exitArena(_playerController, this, false);
		...
		if (_dropManager != null)
		{
			...
			rootNode.detachChild(_dropManager.getNode());
			_dropManager = null;
		}
		...
		if (_particleManager != null)
		{
			...
			rootNode.detachChild(_particleManager.getNode());
		...
		if (_blockInteraction != null)
		{
			...
			rootNode.detachChild(_blockInteraction.getOverlayNode());
			rootNode.detachChild(_blockInteraction.getDestructionEffectsNode());
		...
		{
			rootNode.detachChild(_spawnSystem.getEnemyNode());
			_spawnSystem = null;
		}
		...
		if (_world != null)
		{
			...
			if (tileEntityManager != null)
			{
				rootNode.detachChild(tileEntityManager.getNode());
			}
			
			RegionMeshBuilder.setGlobalTileEntityManager(null);
			_world.shutdown();
			rootNode.detachChild(_world.getWorldNode());
			_world = null;
		}
